    ...
    <div className="flex flex-col h-full bg-slate-50 font-sans">
      ...
      <div className="flex flex-1 overflow-hidden">
          <div className="w-64 bg-white border-r border-slate-200 flex flex-col p-4 space-y-2 z-0">
              ...
              <div className="mt-8 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 px-2">System</div>
              <button className="w-full text-left px-4 py-3 rounded-xl flex items-center gap-3 hover:bg-slate-50 text-slate-400 transition-all cursor-not-allowed">
                  <Terminal size={18}/> JSON Configs (WIP)
              </button>
          </div>
                                      ...
                                      {currentTabConfig?.columns.map(col => (
                                          <td key={col.key} className="p-4 text-slate-700">
                                              {col.key === 'standard_cost' 
                                                ? <span className="font-bold text-emerald-600">฿ {item[col.key]?.toLocaleString()}</span> 
                                                : col.key === 'status' ? (
                ...
                {activeTab === 'master_products' && (
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                             ...
                             <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Conv. Rate</label><input name="conversion_rate" type="number" defaultValue={currentItem?.conversion_rate || 1} className="w-full p-2.5 border border-slate-300 rounded-xl text-sm" placeholder="1"/></div>
                             <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Min Stock Alert</label><input name="min_stock" type="number" defaultValue={currentItem?.min_stock || 10} className="w-full p-2.5 border border-slate-300 rounded-xl text-sm" placeholder="10"/></div>
                        </div>
